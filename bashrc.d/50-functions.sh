...
zip_single() {
  while [ "$#" -gt 0 ]; do
    case "$1" in
      -h|--help)
        echo "Usage: zip_single [-h|--help] SOURCE zipGET"
        return 0
        ;;
      --)
        shift
      ...
      -*)
        echo "Unknown option: $1" >&2
        echo "Usage: zip_single [-h|--help] SOURCE zipGET" >&2
        return 1
        ;;
      *)
        break
  ...
  [ "$#" -eq 2 ] || { echo "Usage: zip_single [-h|--help] SOURCE zipGET" >&2; return 2; }
...
zip_split() {
  ...
  while [ "$#" -gt 0 ]; do
    case "$1" in
      -h|--help)
        echo "Usage: zip_split [-h|--help] [-b SIZE | --bytes=SIZE] SOURCE zipGET"
        echo 'Default size: $SPLIT_SIZE if set and 4000M if not'
        return 0
        ;;
      -b)
      ...
      -*)
        echo "Unknown option: $1" >&2
        echo "Usage: zip_split [-h|--help] [-b SIZE | --bytes=SIZE] SOURCE zipGET" >&2
        echo 'Default size: $SPLIT_SIZE if set and 4000M if not' >&2
        return 1
        ;;
      *)
  ...
  [ "$#" -eq 2 ] || { echo "Usage: zip_split [-h|--help] [-b SIZE | --bytes=SIZE] SOURCE zipGET" >&2; echo 'Default size: $SPLIT_SIZE if set and 4000M if not' >&2; return 2; }
