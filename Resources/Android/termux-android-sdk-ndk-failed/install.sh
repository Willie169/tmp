...
export TOOLCHAIN="$ANDROID_NDK_HOME/toolchains/llvm/prebuilt/linux-aarch64"
export PATH="$JAVA_HOME/bin:$ANDROID_SDK_ROOT/cmdline-tools/latest/bin:$ANDROID_SDK_ROOT/platform-tools:$ANDROID_NDK_HOME:$TOOLCHAIN/bin:$PATH"
EOF
source ~/.bashrc
pkg update
...
git clone https://github.com/lzhiyong/android-sdk-tools
cd android-sdk-tools
python get_source.py
find src -name CMakeLists.txt -type f -exec sed -i -E '
/^cmake_minimum_required/{
  s/VERSION[[:space:]]+[0-9.]+/VERSION 3.5...3.27/
}
' {} +
cat >> src/protobuf/CMakeLists.txt << 'EOF'
include_directories(
...
EOF
cat > src/core/libprocessgroup/cgrouprc_format/cgroup_controller.cpp << 'EOF'
/*
...
EOF
cat > src/core/libprocessgroup/cgrouprc/cgroup_controller.cpp << 'EOF'
/*
 * Copyright (C) 2019 The Android Open Source Project
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
...
EOF
cat > src/core/libprocessgroup/cgrouprc/cgroup_file.cpp << 'EOF'
/*
 * Copyright (C) 2019 The Android Open Source Project
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
...
EOF
mkdir -p processgroup
mkdir -p processgroup/format
cat > processgroup/format/cgroup_controller.h << 'EOF'
...
EOF
rm src/adb/client/openscreen/mdns_service_watcher.cpp
aria2c https://android.googlesource.com/platform/packages/modules/adb/+/f4ba8d73079b99532069dbe888a58167b8723d6c/client/openscreen/mdns_service_watcher.cpp -o src/adb/client/openscreen/mdns_service_watcher.cpp
rm src/adb/client/transport_local.cpp
aria2c https://android.googlesource.com/platform/system/core/+/0140182/adb/transport_local.cpp -o src/adb/client/transport_local.cpp
rm -rf src/boringssl
cd src
git clone https://boringssl.googlesource.com/boringssl
cd ..
sed -i -E '
s|(__android_log_logd_logger_with_timestamp\(log_message, nullptr\);)|#if __ANDROID_API__ >= 37\
\1\
#else\
__android_log_write(log_message->priority, log_message->tag, log_message->message);\
#endif|g
' src/logging/liblog/logger_write.cpp
sed -i -E 's/constexpr inline/inline/' src/base/libs/androidfw/include/androidfw/LocaleDataLookup.h
cd src/protobuf
rm -rf build
...
cmake -GNinja \
  ...
  -DPROTOC_PATH=src/protobuf/build/protoc-3.21.12.0 \
  -DCMAKE_CXX_FLAGS="-I/data/data/com.termux/files/home/android-sdk-tools"
ninja -C build/aarch64 aapt
ninja -C build/aarch64 aapt2
ninja -C build/aarch64 aidl
ninja -C build/aarch64 zipalign
ninja -C build/aarch64 dexdump
ninja -C build/aarch64 split-select
mkdir -p ~/.gradle
